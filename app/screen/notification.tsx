...
export default function NotificationsPage() {
    ...
    const markAsRead = async (id: string) => {
        try {
            ...
            Object.keys(updatedNotifications).forEach(key => {
                updatedNotifications[key] = updatedNotifications[key].map(n =>
    ...
    const markAllAsRead = async () => {
        try {
            await notificationService.markAllAsRead(filterType || undefined);
            await loadData();
                ...
                { text: "Annuler", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            const updatedNotifications = { ...notifications };
                            Object.keys(updatedNotifications).forEach(key => {
                                updatedNotifications[key] = updatedNotifications[key].filter(n => n.id !== id);
                                if (updatedNotifications[key].length === 0) {
        ...
        Alert.alert(
            ...
            [
                { text: "Annuler", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            await notificationService.deleteAll(filterType || undefined);
                            await loadData();
    ...
    const archiveOld = async () => {
        try {
            await notificationService.archiveOld();
            await loadData();
    ...
    const filteredNotifications: Record<string, Notification[]> = filterType
        ? Object.keys(notifications).reduce((acc, key) => {
              const filtered = notifications[key].filter(n => n.type === filterType);
    ...
    if (loading) {
        return (
            <ThemedSafeAreaView style={{ backgroundColor: isLight ? "#F5F5F7" : theme.background }}>
    ...
    return (
        <ThemedSafeAreaView style={[styles.container, { backgroundColor: isLight ? "#F5F5F7" : theme.background }]}>
            <ThemedScrollView
                style={{ backgroundColor: isLight ? "#F5F5F7" : theme.background }}
            ...
            >
                {/* Header Moderne */}
                <View style={{ position: 'relative', width: '100%', height: 300, overflow: 'hidden', paddingBottom: 24 }}>
                    <LinearGradient
                    ...
                    <View style={[styles.headerActions, { paddingHorizontal: 24 }]}>
                        <TouchableOpacity style={styles.headerActionButton} onPress={archiveOld}>
                            <Ionicons name="archive-outline" size={18} color="#FFF" />
                            <Text style={styles.headerActionText}>Archiver</Text>
                        </TouchableOpacity>
                        <TouchableOpacity
                        ...
                        >
                            ...
                            <Text style={styles.headerActionText}>Stats</Text>
                        </TouchableOpacity>
                    </View>
                </View>
                ...
                <View style={styles.notificationsContent}>
                    {Object.entries(filteredNotifications).map(([dateKey, notifs]) => (
                            ...
                            {notifs.map((notif) => {
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        onLongPress={() => {
                                            Alert.alert(
                                                ...
                                                [
                                                    ...
                                                    {
                                                        text: notif.isRead ? "Marquer non lu" : "Marquer lu",
                                                        onPress: async () => {
                                        ...
                                        activeOpacity={0.7}
                                    >
                                        <View style={[styles.notifIcon, { backgroundColor: typeConfig.color + "20" }]}>
                                        ...
                                        <View style={[styles.cardIndicator, { backgroundColor: typeConfig.color }]} />
                                    </TouchableOpacity>
                                );
                            })}
                        </View>
